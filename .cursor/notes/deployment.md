...
- Runner stage: `gcr.io/distroless/nodejs24-debian12:nonroot`; OpenSSL tracks distroless rebuilds — use `docker build --pull`; runtime user **65532** (`nonroot`)
...
- **Current production image**: `gcr.io/distroless/nodejs24-debian12:nonroot` — minimal; OpenSSL follows Google’s Debian bookworm rebuilds; use `docker build --pull` to pick up newer digests (may still lag `bookworm-security` briefly).
- **Hybrid option**: keep distroless final stage and `COPY` only `libssl.so.3` / `libcrypto.so.3` from a `debian:bookworm-slim` stage that installs pinned `libssl3`; ABI matches bookworm; risk if distroless glibc/openssl layout diverges from security archive.
