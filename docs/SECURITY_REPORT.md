...
- **1 Moderate dependency vulnerability** requiring fix within 1-2 weeks
...
**IMPORTANT**: This assessment is **ONLY valid for controlled, non-public deployments**. If the service is ever exposed to public internet or untrusted networks, the security rating would drop to **D+ (Critical)** and all previously identified critical issues would require immediate remediation.
...
## SECURITY STATUS DEGRADATION ALERT
...
| **Moderate** | **1** | **+1 NEW** | **NEW VULNERABILITY** |
| **Low** | **0** | No Change | **SECURE** |
| **TOTAL** | **1** | **+1 NEW** | **DEGRADED** |
...
| **Strong Copyleft Licensed Components** | **1** | **0.4%** | **AGPL-3.0 MAIN PROJECT** |
...
|:--------------------------------------|:--------|:------------|:-------------------------------|
| **MIT** | **138** | **58.2%** | Permissive |
| **ISC** | **70** | **29.5%** | Permissive |
| **BSD-3-Clause** | **18** | **7.6%** | Permissive |
| **Apache-2.0** | **3** | **1.3%** | Permissive |
| **(MIT OR CC0-1.0)** | **2** | **0.8%** | Dual License |
| **BSD-2-Clause** | **2** | **0.8%** | Permissive |
| **AGPL-3.0** | **1** | **0.4%** | **Strong Copyleft** (Critical Compliance) |
| **(MIT OR WTFPL)** | **1** | **0.4%** | Dual License |
| **(Apache-2.0 AND MIT)** | **1** | **0.4%** | Multi License |
| **(BSD-2-Clause OR MIT OR Apache-2.0)** | **1** | **0.4%** | Multi License |
| **(MIT AND BSD-3-Clause)** | **1** | **0.4%** | Multi License |
...
```mermaid
%%{init: {'theme':'base', 'themeVariables': {'xyChart': {'plotColorPalette': '#4472C4', 'backgroundColor': '#ffffff'}}}}%%
xychart-beta
...
|:-------------------------------|:--------|:--------|:--------------------------------------|
| @digitalbazaar/ed25519-signature-2020 | ^5.4.0 | MIT | Ed25519 digital signatures |
| @digitalbazaar/ed25519-verification-key-2020 | ^4.2.0 | MIT | Ed25519 key verification |
| @digitalbazaar/vc | ^7.0.0 | MIT | Verifiable credentials |
| @noble/curves | ^1.9.6 | MIT | Elliptic curve cryptography |
| jose | ^6.1.0 | MIT | JSON Web Token operations |
| jsonld-signatures | ^11.5.0 | MIT | JSON-LD digital signatures |
...
- **REQUIRES COMPLIANCE**: Must provide source code to users
- **NETWORK COPYLEFT**: Web service usage triggers obligations
...
3. **AGPL Obligations**: Main project license requires source code disclosure
...
4. **No License Conflicts**: All dependencies compatible with main license
...
1. **Complete Absence of Authentication and Authorization Framework** --- **REEVALUATED FOR CONTROLLED ENVIRONMENT**
   ...
   - **Impact (Controlled Environment Context)**:
     ...
     - **REMAINING RISK**: Insider threats or compromised services within the cluster
     - **REMAINING RISK**: Lack of audit trail for compliance
...
2. **Information Disclosure Through Detailed Error Messages and Stack Traces** --- **REEVALUATED FOR CONTROLLED ENVIRONMENT**
   ...
   - **Vulnerable Code**:
     ...
     logError(`Error ${status}: ${message}`, {
       ...
       stack: exception instanceof Error ? exception.stack : undefined,  // EXPOSES STACK TRACES
     });
     ```
   ...
   - **Impact (Controlled Environment Context)**:
     ...
     - **REMAINING RISK**: Minimal - stack traces may aid compromised internal service in reconnaissance
...
3. **Cryptographic Key Material Exposure Risk Through Logging** --- **REEVALUATED WITH REQUEST ENCRYPTION**
   ...
   - **Vulnerable Code**:
     ...
     const formatMessage = (level: string, message: string, meta?: any): string => {
       ...
       const metaStr = meta ? ` ${JSON.stringify(meta)}` : "";  // LOGS EVERYTHING IN META
   ...
   - **Impact (With Request Encryption Enabled)**:
     ...
     - **REMAINING RISK**: Service-layer logging after decryption could still expose:
     ...
     - **REMAINING RISK**: Response data (public keys, status messages) could be logged, though these are non-sensitive
   - **Risk Assessment**:
     ...
     - **Service Layer Logging**: MEDIUM RISK - Decrypted data present, but limited exposure in controlled environment
...
4. **Database Configuration Security Vulnerabilities** --- **REEVALUATED FOR CONTROLLED ENVIRONMENT**
   ...
   - **Vulnerable Configuration**:
     ...
     export const baseDbConfig: DataSourceOptions = {
       host: process.env.DB_HOST || "localhost",           // Default fallback
       username: process.env.DB_USERNAME || "postgres",    // Default credentials
       password: process.env.DB_PASSWORD || "postgres",    // Weak default password
       ssl: process.env.DB_SSL === "true" ? {
         rejectUnauthorized: false                          // Insecure SSL config
       } : false,
       logging: process.env.NODE_ENV !== "production",     // May log sensitive queries
     };
     ```
   ...
   - **Impact (Controlled Environment Context)**:
     ...
     - **REMAINING RISK**: Default credentials should be changed per security best practices
     - **REMAINING RISK**: `rejectUnauthorized: false` reduces defense-in-depth
     - **REMAINING RISK**: Query logging may expose encrypted key data in non-production environments
...
5. **Missing API Endpoint Access Controls** --- **REEVALUATED FOR CONTROLLED ENVIRONMENT**
   ...
   - **Impact (Controlled Environment Context)**:
     ...
     - **REMAINING RISK**: Any service with the shared encryption key can perform any operation
     - **REMAINING RISK**: No fine-grained authorization (e.g., which service can access which identifiers)
     - **REMAINING RISK**: Limited audit trail for compliance requirements
...
6. **CORS Configuration Weaknesses**
   ...
   - **Configuration Issues**:
     ...
     export const corsConfig: CorsConfig = {
       ...
       } : undefined, // Falls back to allowing ALL origins
     };
     ```
...
| **CORS Configuration** | Medium | Medium | **UNCHANGED** |
| **Rate Limiting** | Partial | Partial | **UNCHANGED** |
...
1. **Fix Dependency Vulnerability** - **STILL REQUIRED**
...
- Dependency vulnerability should be fixed within 1-2 weeks
